...
bool LDPCDecode::extractLDPC(QByteArray &from, std::vector<bool> &extCode)
{
    if(generatormatrixloaded)
    {
        extCode.resize(vars.M);
        for(int i = 0; i < (vars.M); i++)
        {
            extCode[i] = from.at(vars.cols[i]);
            if(from.at(vars.cols[i]))
                extCode[i] = 1;
            else
                extCode[i] = 0;
        }
...
bool LDPCDecode::includeLDPC(std::vector<bool> &from, std::vector<bool> &ldpcCode, QVector<double> &to)
{
    if(generatormatrixloaded)
    {
        to.resize(vars.N);
        for (int i= 0;i<(vars.M);i++)
        {
            to[vars.cols[i]] = ldpcCode[i];
        }
        int  j = 0;
        for (int i= (vars.M);i<(vars.N);i++)
        {
            to[vars.cols[i]]= from[j];
            j++;
        }
